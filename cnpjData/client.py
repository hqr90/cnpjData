...
from .exceptions import RateLimitException, APIException
...
class CNPJAPIClient:
    ...
    def __init__(self):
        self.get_rate_limiter = RateLimiter(max_calls=3, period_seconds=60)  # 3 GET requests per minute
        self.post_rate_limiter = RateLimiter(max_calls=3, period_seconds=60)  # 3 POST requests per minute
    ...
    def _handle_rate_limit(self, response):
        ...
        try:
            ...
            if reset_time:
                ...
                if wait_seconds > 0:
                    print(f"Limite de requisições atingido. Aguardando {wait_seconds:.2f} segundos...")
                    time.sleep(wait_seconds)
                    # Após esperar, tenta novamente a requisição
                    return {"status": 429, "message": "Limite de requisições atingido. Tentando novamente..."}
